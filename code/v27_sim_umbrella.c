#include "21.h"
#include "crystal_coords_v18_for_v27_cutoff0.6.c" // int biggest_cluster_size(int what_order) returns the largest cluster this step
...
const char labelstring[] = "v27_%02d_pf%04.2lf_p%04.1lf_a%04.2lf_%c_t%04d_c%5.3lf_0.6";
...
(readfile / # cubes per dim) (output_folder / packing_fraction) mc_steps output_steps BetaP/NVT Phi order_mode \
target_cluster_size coupling_parameter\norder_mode: transl = 1, sl = 2, unsl = 3, edge = 4\n";
...
int main(int argc, char* argv[])
{
    ...
    if (IsCreated) {
        ...
        sprintf(datafolder_name, buffer_df, CubesPerDim, packing_fraction, BetaP, Phi, order_character, target_cluster_size, coupling_parameter);
        sprintf(densityfile_name, buffer_rho, CubesPerDim, packing_fraction, BetaP, Phi, order_character, target_cluster_size, coupling_parameter);
    } else {
...
// make the folder to store all the data in, if it already exists do nothing.
    ...
    printf("eq. step\tdensity\t  acceptances\t\t  deltas\n");
    ...
    for (int step = 0; step <= mc_steps; ++step) {
        ...
        if (step % 2 == 0) {
            ...
            diff = biggest_cluster_size(what_order) - target_cluster_size;
        ...
        if (step % output_steps == 0) {
            ...
            printf("%d\t%.3lf\t  %.3lf\t%.3lf\t%.3lf\t%.3lf\t  %d\t%-7.2lf\t%d\n",
                step,
                n_particles * ParticleVolume / (sim->box[0] * sim->box[1] * sim->box[2]),
