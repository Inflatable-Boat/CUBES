...
static double BetaP = 1; // start at one, go to 10000 after fluids_steps = 10000 mc_steps
static double Phi; // angle of slanted cube

const char labelstring[] = "v2_%02dpf%04.2lfp%04.1lfa%04.2lf";
...
(readfile / # cubes per dim) mc_steps packing_fraction Phi\n";
...
// make the folder to store all the data in, if it already exists do nothing.
    ...
    for (int step = 0; step <= mc_steps; ++step) {
        ...
        if (step % output_steps == 0) {
            ...
            if (step > fluids_steps) {
                if (density >= packing_fraction) {
                    printf("desired packing fraction reached at step %d\n", step);
                    write_data(step, fp_density, datafolder_name);
                    fclose(fp_density);
                    return 0; // TODO: make NVT at this point
                }
            }
        }
    }
