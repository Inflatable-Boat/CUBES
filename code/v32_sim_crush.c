...
static int mc_steps;
static double packing_fraction = 0.1;
static double BetaP = 1;
static double Phi; // angle of slanted cube

const char labelstring[] = "v32_%02dpf%04.2lfp%04.1lfa%04.2lf";
...
int main(int argc, char* argv[])
{
    ...
    char pack_fracfile_name[128] = "";
    strcat(pack_fracfile_name, datafolder_name);
    strcat(pack_fracfile_name, "/stepnumbers");
    printf("saving to:\n%s\n%s\n%s\n%s...\n", datafolder_name, densityfile_name, gofrfile_name, pack_fracfile_name);
...
// make the folder to store all the data in, if it already exists do nothing.
    ...
    FILE* fp_stepnumbers = fopen(pack_fracfile_name, "w");
    ...
    const int MAX_RAN = 2 * n_particles + 2;

    printf("eq. step\tdensity\t  acceptances\t\t  deltas\n");

    for (int step = 0; step <= 15000; step++) { // 15000 equilibration sweeps
        ...
        if (step % 50 == 0) {
            ...
            printf("%d\t\t%.4lf\t  %.3lf\t%.3lf\t%.3lf\t  %.3lf\t%.3lf\t%.3lf\n",
                step,
    ...
    for (int step = 0; step <= mc_steps; ++step) {
        ...
        if (step > fluids_steps) {
            if (step % output_steps == 0) {
                double density = n_particles * ParticleVolume / (sim->box[0] * sim->box[1] * sim->box[2]);
    ...
    fclose(fp_stepnumbers); // datafolder/.../stepnumber
    free(sim);
