...
//////////////////////////////////////////////////////////////// my additions
const char usage_string[] = "usage: program.exe order_cutoff datafolder read_per output_per \
...
static double order_cutoff;
static char order_cutoff_char;
//////////////////////////////////////////////////////////////// end of my additions
...
void save_cluss(int step, int* cluss, int* size, int big, int nn, int mode, compl_t* orderp)
{
    ...
    if (mode & transl) {
        ...
        strcat(buffern, "/v33_transl");
    } else if (mode & orient) {
        strcat(buffer, "/clust_orient");
        strcat(buffern, "/v33_orient");
        if (order_mode & axes_slanted_normals) {
    ...
    char temp[2]; // this is a done in such a hacky way because I made bad choices
    temp[0] = order_cutoff_char;
    temp[1] = '\0';
    strcat(buffer, temp);
    strcat(buffern, temp);
...
int parse_commandline(int argc, char* argv[])
{
    ...
    if (EOF == sscanf(argv[1], "%lf", &order_cutoff)) {
        printf("reading order_cutoff has failed\n");
        return 1;
    }
    bnd_cuttoff = order_cutoff * 0.1;
    order_cutoff_char = (char) ((int) order_cutoff) + '0';
    //DEBUG
    printf("order_cutoff = %lf\n", order_cutoff);
    printf("bnd_cuttoff = %lf\n", bnd_cuttoff);
    printf("order_cutoff_char = %c\n", order_cutoff_char);
    // exit(EXIT_SUCCESS);
    //ENDEBUG
    if (EOF == sscanf(argv[2], "%s", datafolder_name)) {
        printf("reading datafolder_name has failed\n");
        return 1;
    }
    if (EOF == sscanf(argv[3], "%d", &read_per)) {
        printf("reading read_per has failed\n");
        return 1;
    }
    output_per = 0;
    if (EOF == sscanf(argv[4], "%d", &output_per)) {
        printf("reading output_per has failed\n");
        return 1;
    }
    if (order_cutoff <= 1 || order_cutoff >= 10) {
        printf("1 < order_cutoff < 10, order_cutoff will be divided by 10 to give a number between 0.1 and 1.\n");
        return 2;
    }
    if (read_per <= 0) {
        printf("read_per >= 1\n");
...
you want to print at other intervals than possible. Exiting\n");
    ...
    if (strcmp(argv[5], "t") == 0 || strcmp(argv[5], "transl") == 0 || strcmp(argv[5], "translation") == 0
        || strcmp(argv[5], "p") == 0 || strcmp(argv[5], "pos") == 0 || strcmp(argv[5], "position") == 0) {
        printf("doing translational ordering\n");
        order_mode = transl;
    } else if (strcmp(argv[5], "o") == 0 || strcmp(argv[5], "orient") == 0 || strcmp(argv[5], "orientation") == 0) {
        printf("doing orientational ordering\n");
        order_mode = orient;
    } else if (strcmp(argv[5], "b") == 0 || strcmp(argv[5], "both") == 0) {
        printf("doing both translational and orientational ordering\n");
        order_mode = transl | orient;
    } else if (strcmp(argv[5], "a") == 0 || strcmp(argv[5], "all") == 0) {
    ...
    if (order_mode & orient) {
        if (strcmp(argv[6], "s") == 0 || strcmp(argv[6], "sl") == 0 || strcmp(argv[6], "sl_norm") == 0 || strcmp(argv[6], "slanted_normals") == 0) {
            printf("taking slanted normals as axes for orientational ordering\n");
            order_mode |= axes_slanted_normals;
        } else if (strcmp(argv[6], "u") == 0 || strcmp(argv[6], "unsl") == 0 || strcmp(argv[6], "unsl_norm") == 0 || strcmp(argv[6], "unslanted_normals") == 0) {
            printf("taking unslanted normals as axes for orientational ordering\n");
            order_mode |= axes_unslanted_normals;
        } else if (strcmp(argv[6], "e") == 0 || strcmp(argv[6], "edge") == 0 || strcmp(argv[6], "edges") == 0) {
