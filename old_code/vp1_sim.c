...
const char labelstring[] = "vp1_%02dpf%04.2lfp%04.1lfa%04.2lf";
// e.g. sl10pf0.50p08.0a1.25:
// 10 CubesPerDim, pack_frac 0.50, pressure 8.0, angle 1.25
const char usage_string[] = "usage: program.exe (read / create) (readfile / CubesPerDim) \
(output_folder / packing_fraction) mc_steps output_steps BetaP/NVT Phi aspect_ratio\n";
...
void read_data2(char* init_file)
{
    ...
    SinPhi = sin(Phi);
    CosPhi = cos(Phi);
    ParticleVolume = SinPhi * aspect_ratio; // Edge_Length == 1
...
void create_system(void)
{
    ...
    for (int i = 0; i < n_particles; i++) {
        ...
        m[i].m[0][0] = aspect_ratio; // x - axis of rectangular prism is different 
    }
}
...
void set_packing_fraction(void)
{
    ...
    double optimal_packing_fraction = pow(aspect_ratio * (1.0 + 0.5 * CosPhi), -3.0) * 0.95;
...
void remove_overlap_smart(void)
{
    ...
    if (n_particles * ParticleVolume / (box[0] * box[1] * box[2]) > pow(aspect_ratio * (1.0 + 0.5 * CosPhi), -3.0) * 0.96) {
...
int parse_commandline(int argc, char* argv[])
{
    ...
    if (EOF == sscanf(argv[8], "%lf", &aspect_ratio)) {
        printf("reading Phi has failed\n");
        return 1;
    };
