...
void save_cluss(int step, int* cluss, int* size, int big, int nn, int mode, compl_t* orderp)
{
    ...
    if (mode & transl) {
        ...
        strcat(buffern, "/v20_transl");
    } else if (mode & orient) {
        strcat(buffer, "/clust_orient");
        strcat(buffern, "/v20_orient");
        if (order_mode & axes_slanted_normals) {
    ...
    // now make and print the clustersize histogram
    {
        // make local scope to avoid clutter of random variables
        // make a histogram of number of neighbours:
        const int NBINS = 100;
        int clsz_hist[NBINS];
        memset(clsz_hist, 0, (NBINS) * sizeof(*clsz_hist));
        for (int i = 1; i < nn; i++) {
            if (size[i] > NBINS) {
                printf("size[%d] = %d, too large\n", i, size[i]);
                exit(8); //TODO
            } else {
                clsz_hist[size[i] - 1]++;
            }
        ...
        // double oneovern_particles = 1.0 / n_particles;
        // for (int i = 0; i < NBINS; i++) {
        //     clsz_hist[i] *= oneovern_particles;
        // }
        for (int i = 0; i < NBINS; i++) {
            fprintf(data_file, "%d ", clsz_hist[i]);
        }
        fprintf(data_file, "\n");
    }
