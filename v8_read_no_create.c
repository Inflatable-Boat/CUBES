...
const double MARGIN_OF_ERROR = 0.005;

// const char labelstring[] = "v8_%02dpf%04.2lfp%04.1lfa%04.2lf";
...
// void print_distances_between(int i, int j);
...
int main(int argc, char* argv[])
{
    ...
    double actual_density = n_particles * ParticleVolume / (box[0] * box[1] * box[2]);
    if (fabs(Density - actual_density) > MARGIN_OF_ERROR) {
        printf("\n\n\tWARNING\n\n\nThe specified density %lf is different \
from the actual density %lf.\nExiting.", Density, actual_density);
        return 2;
    } else {
        printf("Specified density: %lf\nActual density: %lf\nDifference: %lf, smaller than margin of error \
%5.3lf, continuing.\n", Density, actual_density, fabs(Density - actual_density), MARGIN_OF_ERROR);
    }

    int overlap_status = 0;
    if (is_overlap()) {
        printf("\n\n\tWARNING\n\n\nThe read file contains overlap.\n\
Continuing but returning nonzero exit status as warning\n");
        overlap_status = 7;
    } else {
...
// make the folder to store all the data in, if it already exists do nothing.
    ...
    for (int step = 0; step <= mc_steps; ++step) {
        for (int n = 0; n < 2 * n_particles + 1; ++n) {
            ...
            int temp_ran = (int)ran(0, 2 * n_particles + 1); // make +1 into +2 if also want volume moves
            if (temp_ran < n_particles) {
                ...
                vol_attempted++; // if also want volume moves make +1 into +2 on temp_ran line
    ...
    for (int i = 0; i < n_particles; i++) {
        for (int j = i + 1; j < n_particles; j++) {
            if (is_overlap_between(i, j)) {
                printf("overlap between particle %d and %d\n", i, j);
                // print_distances_between(i, j);
                return true;
            }
        }
    }
...
// usage: program.exe nvt datafolder/<input> StepNumber mc_steps datafolder/<output>
    ...
    if (strcmp(argv[1], "nvt") == 0 || strcmp(argv[1], "NVT") == 0) {
        if (EOF == sscanf(argv[2], "%lf", &Density)) {
            printf("reading Density has failed\n");
            return 1;
        };
        if (EOF == sscanf(argv[4], "%d", &StepNumber)) {
        ...
        // IsCreated = false;
    } else if (strcmp(argv[1], "npt") == 0 || strcmp(argv[1], "NpT") == 0 || strcmp(argv[1], "NPT") == 0) {
    ...
    if (EOF == sscanf(argv[5], "%d", &mc_steps)) {
        printf("reading mc_steps has failed\n");
        return 1;
    };
    // if (EOF == sscanf(argv[6], "%s", &output_foldername)) {
...
/// DEBUG method
/// prints distance between centers;
// ./v8_read_no_create_debug nvt 0.49 v6_20a1.57 283800 100000 v8_20pf0.49a1.57
/* void print_distances_between(int i, int j)
{
    vec3_t r2_r1 = v3_sub(r[i], r[j]);
    double* pdist = &(r2_r1.x);
    for (int d = 0; d < NDIM; d++) {
        if (*(pdist + d) > 0.5 * box[d])
            *(pdist + d) -= box[d];
    ...
    printf("distance between centers of %d and %d is %lf\n", i, j, v3_length(r2_r1));

    // now calculate collision depth
    ...
    for (int k = 0; k < 15; k++) {
        if (!is_collision_along_axis(axes[k], i, j, r2_r1))
            printf("no overlap on axis %d\n", k); // found separation, no overlap!
        else
            printf("overlap on axis %d!!!\n", k); // found overlap
    }
    ...
    if (max1 < min2 || max2 < min1) {
        ...
        printf("collision:\nmin1: %lf\nmax2: %lf\nmin2: %lf\nmax1: %lf\n", min1, max2, min2, max1);
        printf("max1 - min2 = %lf\nmax2 - min1 = %lf\n", max1 - min2, max2 - min1);
        // if (max1 < min2) printf("max1 < min2\n");
        if (max1 == min2) printf("max2 < min1\n");
        // if (max1 < min2 || max2 < min1)
            // printf("max1 < min2 or max2 < min1\n");
    ...
    //     }
    //     if (max1 < min2 || max2 < min1) {
    //         printf("no col in axis %d\n", a);
    //         // return false; // separation!
    //     } else {
    //         printf("collision depth in axis %d %lf or %lf\n", a, min2 - max1, min1 - max2);
    //         // return true; // collision
    //     }
    // }
} */